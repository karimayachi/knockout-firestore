...
| object | type: [Function] <br><br>A Javascript function that acts as a Model. All koObservable properties of this function will be synced with document properties of the same name.<br><br>For instance: <br><pre>var BlogPost = function() {<br>   this.title = ko.Observable();<br>   this.content = ko.Observable();<br>}</pre><br><br>Syncs with a Firestore document:<br><pre>{<br>   title: 'foo',<br>   content: 'bar'<br>}</pre>|
